      ...
      {document ? (
        document.map((data) => (
          <Link to={`/articles/${data.id}`} key={data.id}>
            <div className={`mt-5 my-10 ${location.pathname.includes('articles') && "flex items-center justify-between"}`}>
              <div>
              ...
              {location.pathname.includes('articles') && <img src={data.image} alt={data.title} className="w-20 rounded" />}
            </div>
          </Link>
        ))
      ) : (
